...
def createTables():
    ...
    CoordSystems= Table('CoordSystems', metadata,
    ...
    #     (If we want to include dimensions into the framework
    ...
    #     even scalar variables refer to a coordinate system 
    #     ( often just an ordering of statevariables )
    #     so a reference to this coordsymtem has to be stored 
    ...
    DerivedVariables= Table(
        ...
        ,Column('coord_system_id', String)
        ,Column('execution_order', Integer)
    	,ForeignKeyConstraint(['symbol', 'model_id'], ['Variables.symbol', 'Variables.model_id'])
    	,ForeignKeyConstraint(['coord_system_id','model_id'], ['CoordSystems.id', 'CoordSystems.model_id'])
    ...
    IndexedComponents= Table('IndexedComponents', metadata,
        ...
        ,Column('model_id'  ,                primary_key=True )
        ,Column('coord_system_id', String(1000), primary_key=True)
    	,ForeignKeyConstraint(['symbol', 'model_id'], ['Variables.symbol', 'Variables.model_id'])
    	,ForeignKeyConstraint(['model_id','coord_system_id'], ['CoordSystems.model_id', 'CoordSystems.id'])
    ...
    StateVectorPositions= Table('StateVectorPositions', metadata,
    	...
    	,Column('model_id'      , primary_key=True)
    	,Column('coord_system_id'   , primary_key=True)
    	,ForeignKeyConstraint(['symbol', 'model_id'], ['Variables.symbol', 'Variables.model_id'])
    	,ForeignKeyConstraint([ 'model_id','coord_system_id'], ['CoordSystems.model_id', 'CoordSystems.id'])
    )
