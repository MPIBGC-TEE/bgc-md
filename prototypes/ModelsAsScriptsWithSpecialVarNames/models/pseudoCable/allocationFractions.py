def allocationFractions(
    ...
    if (phase==2)  and  (Npp>=0)  and (leaf*sla<glaimax):
        ...
        (bvec_leaf,bvec_wood,bvec_fine_root)=(leaf/(leaf+wood+fine_root),wood/(leaf+wood+fine_root),fine_root/(leaf+wood+fine_root))

    elif (Npp<0) and (leaf+wood+fine_root <=0):     
        (bvec_leaf,bvec_wood,bvec_fine_root)=(r_leaf/(r_leaf+r_wood+r_fine_root),r_wood/(r_leaf+r_wood+r_fine_root),r_fine_root/(r_leaf+r_wood+r_fine_root))
