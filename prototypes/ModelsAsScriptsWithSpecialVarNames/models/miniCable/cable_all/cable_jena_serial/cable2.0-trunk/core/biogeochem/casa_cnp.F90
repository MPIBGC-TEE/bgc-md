...
!        leaf area
  ...
  CHARACTER(LEN=*),PARAMETER      :: FMT='(I1,A,F16.9)'
  ! initlization
...
!endif
  ...
  DO npt=1,mp
     write(408,FMT) npt,',',casaflux%Crmplant(npt,leaf)
     write(409,FMT) npt,',',casaflux%Crmplant(npt,wood)
     write(410,FMT) npt,',',casaflux%Crmplant(npt,froot)
  END DO
...
!     xkleaf(mp):      set to 0.0 during maximal leaf growth phase
  ...
  !    calculate cold stress (eqn (18), Arora 2005, GCB 11:39-59)
    ...
    ELSE
      ...
      ENDIF
    ENDIF
    write(402,FMT) npt,',',casamet%tairk(npt)
...
SUBROUTINE casa_xratesoil(xklitter,xksoil,veg,soil,casamet,casabiome,phen)
...
!,tsurfavg  !!, msurfavg
  ...
  ! BP changed the WHERE construct to DO-IF for Mk3L (jun2010)
  if(write_para .eqv. .true.)then
     write(301,'(A,18(A,A16))') 'npt',','                                               &
             ,'phen%TKshed',',','xkoptlitter',','                                       & 
             ,'xkoptsoil',',','xkleafdrymax',','                                        &
             ,'xkleafdryexp',',','xkleafcoldmax',','                                    &
             ,'xkleafcoldexp',',','q10soil',','                                         &
             ,'wfpscoefa',',','wfpscoefb',','                                           &
             ,'wfpscoefc',',','wfpscoefd',','                                           &
             ,'wfpscoefe',',','soil%ssat',','                                           &
             ,'soil%sfc',',','soil%swilt',','                                           &
  ...
  DO npt=1,mp
     write(403,FMT) npt,',',tsavg(npt)
     write(404,FMT) npt,',',casamet%moistavg(npt)
     if(write_para .eqv. .true.)then
        write(301,'(I4,18(A,F16.9))') npt,','                                                            &
                ,phen%TKshed(veg%iveg(npt)),',',casabiome%xkoptlitter(veg%iveg(npt)),','                &
                ,casabiome%xkoptsoil(veg%iveg(npt)),',',casabiome%xkleafdrymax(veg%iveg(npt)),','       &
                ,casabiome%xkleafdryexp(veg%iveg(npt)),',',casabiome%xkleafcoldmax(veg%iveg(npt)),','   &
                ,casabiome%xkleafcoldexp(veg%iveg(npt)),',',casabiome%q10soil(veg%iveg(npt)),','        &
                ,wfpscoefa,',',wfpscoefb,','                                                            &
                ,wfpscoefc,',',wfpscoefd,','                                                            &
                ,wfpscoefe,',',soil%ssat(npt),','                                                       &
...
!     fromPtoL(mp,mlitter,mplant): fraction of senesced plant biomass to litter pool (fraction)
  ...
  if(write_para)then
     write(302,'(A,11(A,A16))') 'npt',',','fracLigninleaf',','                  &
             ,'fracLigninfroot',',','fracLigninwood',','                        &
             ,'ratioLigninleaf',',','ratioLigninfroot',','                      &
             ,'sla',',','glaimax',','                                           &
             ,'b_leaf',',','b_wood',','                                         &
             ,'b_fine_root',',','planttype'
  end if
  DO npt = 1,mp
     if(write_para)then
        write(302,'(I4,10(A,F16.9),A,I16)') npt,',',casabiome%fracLigninplant(veg%iveg(npt),leaf),',',&
                                             casabiome%fracLigninplant(veg%iveg(npt),froot),',',&
                                             casabiome%fracLigninplant(veg%iveg(npt),wood),',',&
                                             ratioLignintoN(npt,leaf),',',&
                                             ratioLignintoN(npt,froot),',',&
                                             casabiome%sla(veg%iveg(npt)),',',&
                                             casabiome%glaimax(veg%iveg(npt)),',',&
                                             casabiome%fracnpptop(veg%iveg(npt),leaf),',',&
                                             casabiome%fracnpptop(veg%iveg(npt),wood),',',&
                                             casabiome%fracnpptop(veg%iveg(npt),froot),',',&
                                             casamet%lnonwood(npt)
     end if
...
!    casaflux%fromStoS(:,mic,pass)  = 0.45
  ...
  ENDWHERE
    ...
    print*,'fromLtoS',casaflux%fromPtoL(:,metb,leaf)  * casaflux%kplant (:,leaf)  &
                     ,casaflux%fromPtoL(:,metb,froot) * casaflux%kplant (:,froot) & 
                     ,casaflux%fromPtoL(:,str,leaf)   * casaflux%kplant (:,leaf)  &
                     ,casaflux%fromPtoL(:,str,froot)  * casaflux%kplant (:,froot) &
                     ,casaflux%fromPtoL(:,cwd,wood)   * casaflux%kplant (:,wood)  &
                     ,casaflux%fromLtoS(:,mic,metb)   * casaflux%klitter(:,metb)  &
                     ,casaflux%fromLtoS(:,mic,str)    * casaflux%klitter(:,str)   &
                     ,casaflux%fromLtoS(:,slow,str)   * casaflux%klitter(:,str)   & 
                     ,casaflux%fromLtoS(:,mic,cwd)    * casaflux%klitter(:,cwd)   &
                     ,casaflux%fromLtoS(:,slow,cwd)   * casaflux%klitter(:,cwd)   &
                     ,casaflux%fromStoS(:,slow,mic)   * casaflux%ksoil  (:,mic)   &
                     ,casaflux%fromStoS(:,pass,mic)   * casaflux%ksoil  (:,mic)   &
                     ,casaflux%fromStoS(:,pass,slow)  * casaflux%ksoil  (:,slow)
  DO nland=1,mp
  ...
  if(write_para)then
     ...
     do nland = 1,mp
        write(303,'(I4,9(A,F16.9))') nland,',',casabiome%plantrate(veg%iveg(nland),leaf),&
                                          ',',casabiome%plantrate(veg%iveg(nland),wood),&
                                          ',',casabiome%plantrate(veg%iveg(nland),froot),&
                                          ',',casabiome%litterrate(veg%iveg(nland),metb),&
                                          ',',casabiome%litterrate(veg%iveg(nland),str),&
                                          ',',casabiome%litterrate(veg%iveg(nland),cwd),&
                                          ',',casabiome%soilrate(veg%iveg(nland),mic),&
                                          ',',casabiome%soilrate(veg%iveg(nland),slow),&
                                          ',',casabiome%soilrate(veg%iveg(nland),pass)
     end do
  end if
  write_para = .false.
...
SUBROUTINE casa_delplant(veg,casabiome,casapool,casaflux,casamet,            &
  ...
  DO npt=1,mp
     IF(casamet%iveg2(npt)/=icewater) THEN
        ...
        write(406,FMT) npt,',',casaflux%Cnpp(npt)

        write(510,FMT) npt,',',casaflux%fracCalloc(npt,leaf)
        write(511,FMT) npt,',',casaflux%fracCalloc(npt,wood)
        write(512,FMT) npt,',',casaflux%fracCalloc(npt,froot)

        write(513,FMT) npt,',',casaflux%kplant(npt,leaf)
        write(514,FMT) npt,',',casaflux%kplant(npt,wood)
        write(515,FMT) npt,',',casaflux%kplant(npt,froot)
        write(516,FMT) npt,',',casapool%cplant(npt,leaf)
        write(517,FMT) npt,',',casapool%cplant(npt,wood)
        write(518,FMT) npt,',',casapool%cplant(npt,froot)
...
IF(casamet%iveg2(nland)/=icewater) THEN
   ...
   write(523,FMT) nland,',',casaflux%klitter(nland,leaf)
   write(524,FMT) nland,',',casaflux%klitter(nland,wood)
   write(525,FMT) nland,',',casaflux%klitter(nland,froot)
   write(526,FMT) nland,',',casapool%clitter(nland,leaf)
   write(527,FMT) nland,',',casapool%clitter(nland,wood)
   write(528,FMT) nland,',',casapool%clitter(nland,froot)

   write(533,FMT) nland,',',casaflux%ksoil  (nland,leaf)
   write(534,FMT) nland,',',casaflux%ksoil  (nland,wood)
   write(535,FMT) nland,',',casaflux%ksoil  (nland,froot)
   write(536,FMT) nland,',',casapool%csoil  (nland,leaf)
   write(537,FMT) nland,',',casapool%csoil  (nland,wood)
   write(538,FMT) nland,',',casapool%csoil  (nland,froot)
...
! of each tile within the land cell
 ...
 ! Ticket#121
  ...
  ENDDO
  DO nland=1,mp
     write(401,FMT) nland,',',casamet%btran(nland)
  END DO
...
! end (Q.Zhang 23/05/2011)
  ...
  ENDWHERE
  DO nland=1,mp
     write(405,FMT) nland,',',xkNlimiting(nland)
  END DO
...
!  PRINT *, 'Within SUBROUTINE phenology, mp = ', mp
  ...
  DO np=1,mp
     write(407,'(I4,A,I16)') np,',',phen%phase(np)
  END DO
