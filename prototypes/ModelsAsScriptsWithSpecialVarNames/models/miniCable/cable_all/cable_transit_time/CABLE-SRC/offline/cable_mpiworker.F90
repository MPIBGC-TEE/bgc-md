...
CONTAINS
   ...
   SUBROUTINE mpidrv_worker (comm,rank)
   ...
   ! MPI:
   INTEGER               :: comm,rank ! MPI communicator for comms with the workers
...
!         if(rank .eq. 16) print*,'before cbm',rank,icycle
         CALL cbm( dels, air, bgc, canopy, met,                             &
                   bal, rad, rough, soil, ssnow,                            &
                   sum_flux, veg ,rank)
   
!         if(rank .eq. 16) print*,'end of cbm,rank',rank
         ...
         IF(icycle >0) THEN
            call bgcdriver( ktau, kstart, kend, dels, met,                  &
                            ...
                            .FALSE., .FALSE. )
         ENDIF 
!         if(rank .eq. 16) print*,'end of bgcdriver,rank',rank
...
!   print*,'before send restart from worker',rank
   IF(output%restart) THEN
   ...
   END IF
!   print*,'after sending restart from worker',rank

   ! MPI: cleanup
   CALL worker_end(icycle, output%restart)
!   print*,'after worker_end',rank
...
!  blen(bidx) = ms * r1len
  ...
  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cisun, displs(bidx), ierr)
  blen(bidx) = r1len

  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cisha, displs(bidx), ierr)
  blen(bidx) = r1len

  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cx1sun, displs(bidx), ierr)
  blen(bidx) = r1len

  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cx1sha, displs(bidx), ierr)
  blen(bidx) = r1len

  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cx2sun, displs(bidx), ierr)
  blen(bidx) = r1len

  bidx = bidx + 1
  CALL MPI_Get_address (canopy%cx2sha, displs(bidx), ierr)
  blen(bidx) = r1len
...
!mcd287  CALL MPI_Reduce (tsize, tsize, 1, MPI_INTEGER, MPI_SUM, 0, comm, ierr)
  !print*,'casa_worker_outtype',tsize,MPI_DATATYPE_NULL,MPI_SUM
  CALL MPI_Reduce (tsize, MPI_DATATYPE_NULL, 1, MPI_INTEGER, MPI_SUM, 0, comm, ierr)
...
!  is no longer writing phen%phase
  ...
  ! MPI: check whether total size of received data equals total
  ! data sent by all the workers
  !print*,'casa_worker',MPI_DATATYPE_NULL,MPI_SUM,tsize
  CALL MPI_Reduce (tsize, MPI_DATATYPE_NULL, 1, MPI_INTEGER, MPI_SUM, 0, comm, ierr)
