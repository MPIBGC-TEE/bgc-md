...
#ifdef NO_CASA_YET
   INTEGER :: ICYCLE
!   if(rank .eq. 16)print*,'before icycle',icycle,rank
   ICYCLE = 0
#endif
...
!   if(rank .eq. 16)print*,'before define canopy',rank
   CALL define_canopy(bal,rad,rough,air,met,dels,ssnow,soil,veg, canopy,rank)
...
!   if(rank .eq. 16)print*,'before define soilsnow',rank
   IF( cable_runtime%um ) THEN
...
!   if(rank .eq. 16)print*,'before update fev',rank
   ! need to adjust fe after soilsnow
   canopy%fev  = canopy%fevc + canopy%fevw
  
!   if(rank .eq. 16)print*,'before update fe',rank
   ! Calculate total latent heat flux:
   canopy%fe = canopy%fev + canopy%fes

!   if(rank .eq. 16)print*,'before update rnet',rank
   ! Calculate net radiation absorbed by soil + veg
   canopy%rnet = canopy%fns + canopy%fnv

!   if(rank .eq. 16)print*,'before updatetrad',rank
   ! Calculate radiative/skin temperature:
   rad%trad = ( ( 1.-rad%transd ) * canopy%tv**4 +                             &
              rad%transd * ssnow%tss**4 )**0.25
!   if(rank .eq. 16)print*,'before update flws',rank,C%sboltz,C%emsoil
   rad%flws = C%sboltz * C%emsoil * ssnow%tss **4
!   if(rank .eq. 16)print*,'after update flws',C%sboltz,C%emsoil
...
!   if(rank .eq. 16 .or. rank .eq. 5)print*,'umexplicit,icycle',cable_runtime%um_explicit,icycle,rank
   IF (.not.cable_runtime%um_explicit .AND. icycle == 0) THEN
!      if(rank .eq. 16) print*,'herehere'
...
!   if(rank .eq. 16)print*,'end of cbm(inside)',rank
