...
SUBROUTINE define_canopy(bal,rad,rough,air,met,dels,ssnow,soil,veg, canopy,rank)
...
!      tlfy             ! leaf temp (K)
   ...
   REAL(r_2), DIMENSION(mp)    :: cansat,fwsoil,ghwet
   REAL(r_2), DIMENSION(mp,mf) :: gbhu,gbhf,tlfx,tlfy,csx,dsx,ecy,hcy,rny
...
!   if(rank .eq. 16)print*,'beginning of define canopy'
   ! assign local ptrs to constants defined in cable_data_module
   CALL point2constants(C)    
!   if(rank .eq. 16)print*,'before updating cansto'

   ! ACCESS version has this statement but elsewhere?
   IF( .NOT. cable_runtime%um)                                                 &
      canopy%cansto =  canopy%oldcansto
 !  if(rank .eq. 16)print*,'before allocating cansat, fwsoil'

 !  ALLOCATE( cansat(mp), fwsoil(mp))
 !  if(rank .eq. 16)print*,'before allocating cxs,dsx'
 !  ALLOCATE( csx(mp,mf),dsx(mp,mf))
 !  if(rank .eq. 16)print*,'before allocating ecy,hcy,rny'
 !  ALLOCATE( ecy(mp,mf), hcy(mp,mf), rny(mp,mf))
!   if(rank .eq. 16)print*,'before allocating ghwet'
!   ALLOCATE( ghwet(mp))
!   if(rank .eq. 16)print*,'before allocating gbhu'
!   ALLOCATE( gbhu(mp,mf))
!   if(rank .eq. 16)print*,'before allocating gbhf'
!   ALLOCATE( gbhf(mp,mf))
!   if(rank .eq. 16)print*,'before allocating tlfx'
!   ALLOCATE( tlfx(mp,mf))
!   if(rank .eq. 16)print*,'before allocating tlfy'
!   ALLOCATE( tlfy(mp,mf))

!   if(rank .eq. 16)print*,'before updating cansat'
...
!   if(rank .eq. 16)print*,'before surfwetness'
...
!   if(rank .eq. 16)print*,'before radiation',rank
...
!   if(rank .eq. 16)print*,'before iter',rank
   DO iter = 1, NITER
      ...
      !if(rank .eq. 6 .or. rank .eq. 8 .or. rank .eq. 13 .or. rank .eq. 5) then
!      if(rank .eq. 16) print*,'before dryLeaf, rank',rank
      CALL dryLeaf( dels, rad, rough, air, met,                                &
                    veg, canopy, soil, ssnow, dsx,                             &
                    fwsoil, tlfx, tlfy, ecy, hcy,                              &
                    rny, gbhu, gbhf, csx, cansat,                              &
                    ghwet,  iter,rank )
!      if(rank .eq. 16) print*,'after dryLeaf, rank',rank
     
      CALL wetLeaf( dels, rad, rough, air, met,                                &
                    veg, canopy, cansat, tlfy,                                 &
                    gbhu, gbhf, ghwet )

      !print*,'after wetLeaf, rank',rank
      ...
      !print*,'after updatetv,rank',rank
      ...
      !print*,'after within Canopy, rank',rank
      ...
      !print*,'after latentheatflux, rank',rank
      ! Soil sensible heat:
...
!print*,'end of define canopy',rank
...
!   REAL, DIMENSION(:,:), POINTER :: gswmin ! min stomatal conductance
   ...
   REAL, DIMENSION(mp,mf) ::  gswmin, gsw_term, lower_limit2  ! local temp var 
...
!       ENDWHERE
       DO i = 1, mp
         IF ( abs_deltlf(i,1) < ABS( deltlfy(i,1) ) .OR. abs_deltlf(i,2) < ABS( deltlfy(i,2) ) ) THEN
            ...
            rdy(i,:)     = rdx(i,:)
            an_y(i,:)    = anx(i,:)
            canopy%cx1sun(i)   = cx1(i,1)
            canopy%cx1sha(i)   = cx1(i,2)
            canopy%cx2sun(i)   = cx2(i,1)
            canopy%cx2sha(i)   = cx2(i,2)
            canopy%cisun(i)     = ci(i,1)
            canopy%cisha(i)     = ci(i,2)
            ! save last values calculated for ssnow%evapfbl
...
!   if(rank .eq. 16)print*,'here8'
!      print*,'iveg(5002)',veg%iveg(5002),veg%frac4(5002)
!      print*,'gswmin,rdx,vcmxt3,vcmxt4,vx3,vx4,fwsoil',gswmin(5002,:),rdx(5002,:),vcmxt3(5002,:),vcmxt4(5002,:),vx3(5002,:),vx4(5002,:),fwsoil(5002)
!      print*,'gsw_term,scalex',gsw_term(5002,:),rad%scalex(5002,:)
!      print*,'an_y(5002,:),rdy(5002,:)',an_y(5002,:),rdy(5002,:)
!      print*,'iveg(5003)',veg%iveg(5003)
!      print*,'gswmin,rdx,vcmxt3,vcmxt4,vx3,vx4,fwsoil',gswmin(5003,:),rdx(5003,:),vcmxt3(5003,:),vcmxt4(5003,:),vx3(5003,:),vx4(5003,:),fwsoil(5003)
!      print*,'gsw_term,scalex',gsw_term(5003,:),rad%scalex(5003,:)
!      print*,'an_y(5003,:),rdy(5003,:)',an_y(5003,:),rdy(5003,:)
...
SUBROUTINE photosynthesis( csxz, cx1z, cx2z, gswminz,                          &
                           ...
                           vx4z, xleuningz, vlaiz, deltlfz, ciz, anxz )
   ...
   REAL(r_2), DIMENSION(mp,mf), INTENT(INOUT) :: anxz, ciz
   
   ! local variables
   REAL(r_2), DIMENSION(mp,mf) ::                                              &
      coef0z,coef1z,coef2z, delcxz,                                        &
