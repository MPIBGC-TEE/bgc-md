...
!          CALL prepareFiles(ncciy)
   ...
   print*,'xleafdrymax in master',casabiome%xkleafdrymax
...
SUBROUTINE master_casa_params (comm,casabiome,casapool,casaflux,casamet,&
  ...
  bidx = bidx + 1
  print*,'in master_casa_param,before get address',bidx,displs(bidx),casabiome%xkleafdrymax
  ...
  bidx = bidx + 1
  print*,'before get address xleafdrymax in master',bidx,displs(bidx),casabiome%xkleafdrymax
  CALL MPI_Get_address (casabiome%xkleafdrymax, displs(bidx), ierr) 
print*,'ierr',ierr,displs(bidx)
  blen(bidx) = mvtype * extr2
