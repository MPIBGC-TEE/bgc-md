...
def save_function(pickler, obj):
    if not _locate_function(obj): #, pickler._session):
        ...
        if getattr(pickler, '_recurse', False):
            ...
            globs = globalvars(obj, recurse=True, builtin=True, stack = stack)
           ...
           #        del globs[key]
            ...
            #if obj in globs.values():#  or obj in stack: #goujou: commented out
            #goujou: need to use the id, since 'in' will fail otherwise if globs.values contains an array
            if id(obj) in [id(gv) for gv in globs.values()]:#  or obj in stack:
