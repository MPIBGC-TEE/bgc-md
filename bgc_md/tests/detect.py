...
def globalvars(func, recurse=True, builtin=False, stack = set()):
    ...
    if isfunction(func):
        ...
        for obj in getattr(orig_func, func_closure) or {}:
            _vars = globalvars(obj.cell_contents, recurse, builtin, stack) or {} # goujou: added stack
        ...
        if not recurse:
            ...
            for key in func.copy(): #XXX: unnecessary...?
                ...
                func.update(globalvars(nested_func, True, builtin, stack)) # goujou: added stack
    elif iscode(func):
        globs = vars(getmodule(sum)) if builtin else {}
